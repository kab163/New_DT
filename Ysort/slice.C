...
int
DelaunayTriangulation::FindFaces(float* slices, const int pop) {
  int size = triangles.size();

  for(int i = 0; i < size; i++) {
    for(int s = 0; s < (pop+3); s++) {
      if(slices[2*s] == triangles[i].p1[0] && slices[2*s +1] == triangles[i].p1[1]) {
        faces.push_back(s);
      }
      if(slices[2*s] == triangles[i].p2[0] && slices[2*s +1] == triangles[i].p2[1]) {
        faces.push_back(s);
      }
      if(slices[2*s] == triangles[i].p3[0] && slices[2*s +1] == triangles[i].p3[1]) {
        faces.push_back(s);
      }
    }
  }
...
void
DelaunayTriangulation::VerifyResults (float* slices, const int pop) {
  for(int i = 0; i < triangles.size(); i++) {
...
int main(int argc, char* argv[])
{
  ...
  float *slices = (float*)malloc((pop+3) * DIM * sizeof(float));

  qsort(DTarray, NUMPOINTS, sizeof(Pair), compareY);
  slices[2*pop] = X1; slices[2*pop+1] = Y1;
  slices[2*pop+2] = X2; slices[2*pop+3] = Y2;
  slices[2*pop+4] = X3; slices[2*pop+5] = Y3;
  ...
  for(int i = 0; i < factor; i++) {
    ...
    //create mesh
    for (int i = 0 ; i < pop; i++)
      dt.AddPoint(slices[2*i], slices[2*i+1]);

    //find faces of triangles in mesh
    dt.FindFaces(slices, pop);

    //load cleap mesh
    if(load_mesh_host(pop, slices, mesh, dt) != CLEAP_SUCCESS) {
    ...
    //error check
    //if(i==0)dt.VerifyResults(slices, pop);
